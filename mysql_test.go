...
func TestMySQLContainer(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			container, err := NewMySQL(getMySQLLogger(), tt.config)
			if err != nil {
...
func getMySQLLogger() *slog.Logger {
	debug := false
	...
	logger = slog.New(slog.NewTextHandler(os.Stderr, opts)).With(slog.String("component", "mysql-container"))
	return logger
}
